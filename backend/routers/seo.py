...
async def run_seo_analysis(
    ...
    if active_meta:
        meta_title = active_meta.meta_title
        meta_description = active_meta.meta_description
    else:
        focus_keyword = (post.target_keywords or [post.topic])[0]
        try:
            generated_meta = await generate_meta(
                post.title,
                post.topic,
                focus_keyword,
                full_text[:400],
            )
            meta_title = generated_meta.get("meta_title")
            meta_description = generated_meta.get("meta_description")
        except Exception:
            meta_title = None
            meta_description = None
